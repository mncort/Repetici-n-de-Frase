...
void SortearBolillero2(Bolilleros const& bolilleros, Grupos& grupos)
{
    ...
    int cantequip = agrupos.length() - 1;
    int control = 0;
    bool vof;
    ...
    for (int group=0; group<=7; group++){
        mostrargrup = group + 'A';
        std::cout<< "Grupo: " << mostrargrup << "\n";
        for (int i=0; i<=3; i++){
            std::cout<<grupos.at(group).Equipos.at(i).Nombre<<"\n";
            std::cout<<grupos.at(group).Equipos.at(i).Confederación<<"\n";
        }
    }
}
...
{
    int cantsorteos (asortear.length());
